...
impl DataSet {
    ...
    pub fn to_table(&self) -> Table {
        ...
        // Add header
        let headers: Vec<_> = self.0.get_column_names().into_iter().map(|name| Cell::new(name)).collect();
        ...
        for row_idx in 0..height.min(1000) {  // Limit to 1000 rows for display
    ...
    pub fn to_json(&self) -> Result<String> {
        let records = self.0.iter().map(|row| {
...
mod tests {
    ...
    async fn test_json_query() {
        ...
        let sql = format!("SELECT name, score FROM file://{} WHERE age > 20", temp_file.display());
        let result = query(&sql).await;
        assert!(result.is_ok());
